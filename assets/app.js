...
const api = {
  ...
  async getClasses(){
    if(sb){
      ...
      throw new Error("Base en ligne : classes non configurées (mode démo conseillé pour le rendu).");
    }
    const db = ensureDemoDb();
    return (db.classes||[]).slice().sort((a,b)=>a.name.localeCompare(b.name,"fr"));
  },
  async createClass(name){
    if(sb) throw new Error("Base en ligne : classes non configurées.");
    const db = ensureDemoDb();
  ...
  async getTeachers(){
    if(sb) throw new Error("Base en ligne : profs non configurés.");
    const db = ensureDemoDb();
  ...
  async createAssignment({class_id, title, detail, due_at, published, author}){
    if(sb) throw new Error("Base en ligne : devoirs non configurés.");
    const db = ensureDemoDb();
  ...
  async sendMessage({from_email, from_role, from_student_id, to_type, to_value, text, files_meta}){
    if(sb) throw new Error("Base en ligne : messagerie non configurée.");
    const db = ensureDemoDb();
